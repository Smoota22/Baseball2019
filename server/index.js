...
function load_player_stats(req, res) {
    let sql = 'SELECT * FROM ' + req.params.table + ' WHERE ID = "' + req.params.playerID + '" AND year_ID = ' + req.params.yearID + ' AND stint = ' + req.params.stint + ' AND team_ID = "' + req.params.teamID + '" AND league_ID = "' + req.params.leagueID + '"';
    // res.send(sql)
    let query = db.query(sql, (err, results) => {
        if(err) throw err;
        console.log(results);
