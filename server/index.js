...
function autofill_team_names(req, res) {
    ...
    if (req.params.teamName != "NULL") {
        sql = 'SELECT team_name, team_ID FROM real_team WHERE team_name LIKE "' + regex + '" GROUP BY team_name, team_ID';
    }
    // res.send(sql);
    let query = db.query(sql, (err, results) => {
        if(err) throw err;
...
app.get('/autofill_player_teamID/:playerID/:yearID/:stint/:teamName', autofill_player_teamID);
function autofill_player_teamID(req, res) {
    ...
    sql = 'SELECT team_ID,team_name FROM real_team WHERE team_ID IN (' + sql + ') AND year_ID = ' + req.params.yearID;
    // res.send(sql);
    let query = db.query(sql, (err, results) => {
        if(err) throw err;
        console.log(results);
...
function load_player_stats(req, res) {
    let sql = 'SELECT * FROM ' + req.params.table + ' WHERE player_ID = "' + req.params.playerID + '" AND year_ID = ' + req.params.yearID + ' AND stint = ' + req.params.stint + ' AND team_ID = "' + req.params.teamID + '" AND league_ID = "' + req.params.leagueID + '"';
    // res.send(sql)
    let query = db.query(sql, (err, results) => {
        if(err) throw err;
        console.log(results);
